...
unsigned volatile int xspeed = 5;
unsigned volatile int yspeed = 2;
...
void main(void) /*Huvudfunktionen main skapar två pekare till objekten victim (bollen) och creature (spindeln).*/
{
    ...
    victim->set_speed(victim, xspeed, yspeed);
	 ...
	 * Bryter loopen om spindeln kolliderar med kanten av skärmen eller med bollen, lives minskar med ett och spelet återgår till startpositioner.
	 * Om lives är noll avslutas spelet.
	 * */
		if (score >= 100) {
			xspeed = 7;
			yspeed = 3;
